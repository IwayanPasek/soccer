...
// Fix 1: Pastikan konstanta _MPDFK tersedia
if (!defined('_MPDFK')) {
    define('_MPDFK', 2.8346456692913); // (72 / 25.4)
}
...
class grad {
    ...
    function CoonsPatchMesh($x, $y, $w, $h, $patch_array=array(), $x_min=0, $x_max=1, $y_min=0, $y_max=1, $colspace='RGB', $return=false){
        ...
        for($i=0; $i<$patchCount; $i++){
            ...
            for($j=0; $j<$pointsCount; $j++){
                ...
                // Fix 3: Tambahkan (int) untuk PHP 8.1+
                $this->mpdf->gradients[$n]['stream'].=chr((int)floor($patch_array[$i]['points'][$j]/256));
                $this->mpdf->gradients[$n]['stream'].=chr((int)floor($patch_array[$i]['points'][$j]%256));
            }
            
            $colorsCount = count($patch_array[$i]['colors']);
            for($j=0; $j<$colorsCount; $j++){
                ...
                if ($colspace=='RGB') {
                    ...
                    $this->mpdf->gradients[$n]['stream'].=chr((int)(ord($patch_array[$i]['colors'][$j][1])*2.55));
                    $this->mpdf->gradients[$n]['stream'].=chr((int)(ord($patch_array[$i]['colors'][$j][2])*2.55));
                    $this->mpdf->gradients[$n]['stream'].=chr((int)(ord($patch_array[$i]['colors'][$j][3])*2.55));
                    $this->mpdf->gradients[$n]['stream'].=chr((int)(ord($patch_array[$i]['colors'][$j][4])*2.55));
        ...
        if ($trans) {
            ...
            for($i=0; $i<$patchCount; $i++){
                ...
                for($j=0; $j<$pointsCount; $j++){
                    //each point as 16 bit
                    $this->mpdf->gradients[$n]['stream_trans'].=chr((int)floor($patch_array[$i]['points'][$j]/256));
                    $this->mpdf->gradients[$n]['stream_trans'].=chr((int)floor($patch_array[$i]['points'][$j]%256));
                }
    ...
    // $col = array(R,G,B/255);
    // or array(G/255); or array(C,M,Y,K/100)
    ...
    function Gradient($x, $y, $w, $h, $type, $stops=array(), $colorspace='RGB', $coords='', $extend='', $return=false, $is_mask=false) {
        ...
        if ($type == 2) {
            ...
            // If both a <point> and <angle> are defined, the gradient axis starts from the point and runs along the angle.
            // The end point is defined as before - in this case start points may not be in corners, and axis may not correctly fall in the right quadrant.
            ...
            // axis goes through the box.
            // The axis runs along the specified angle. The end point of the axis is defined such that the
    ...
    function parseBackgroundGradient($bg) {
        ...
        if (count($bgr)> 6) {
            if (strtoupper(substr($bgr[0],0,1)) == 'L' && count($bgr)==7) {  // linear
                ...
                // 0 0 1 0 or 0 1 1 1 is L 2 R; 1,1,0,1 is R2L; 1,1,1,0 is T2B; 1,0,1,1 is B2T
                // Linear: $coords - array of the form (x1, y1, x2, y2) which defines the gradient vector (see linear_gradient_coords.jpg).
                //    The default value is from left to right (x1=0, y1=0, x2=1, y2=0).
